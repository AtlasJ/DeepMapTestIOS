...
/**
 Protocol describes set of Map Annotaioning methods
 */
...
/** 
 Add the specified annotation to the map view.
...
/**
 Add the specified annotation to passed coordinates on the map view.

 @param annotation The annotation object to add to the receiver. This object must conform
 to the HDMAnnotation protocol. The map view retains the specified annotation.
 @param coordinate Requred annotation's coordinates
 */
- (void)addAnnotation:(HDMAnnotation * _Nullable)annotation toCoordinate:(HDMMapCoordinate)coordinate;

/**
 Add the specified annotation to passed map entity on the map view.

 @param annotation The annotation object to add to the receiver. This object must conform
 to the HDMAnnotation protocol. The map view retains the specified annotation.
 @param featureId Feature ID of map entity which will be used for adding specified annotation
 
 @note Annotation will be added to bounds' center of object with specified featureId
 */
- (void)addAnnotation:(HDMAnnotation * _Nullable)annotation toEntity:(uint64_t)featureId NS_SWIFT_NAME(add(_:to:));
...
/**
 ...
 @param annotation The annotation object to add to the receiver. This object must conform
 to the HDMAnnotation protocol. The map view retains the specified annotation.
 @param feature Map feature which will be used for adding specified annotation

 @note Annotation will be added to bounds' center specified map feature
 */
- (void)addAnnotation:(HDMAnnotation * _Nullable)annotation toFeature:(HDMFeature* _Nonnull)feature;

/** 
 Add an array of annotations to the map view.
 ...
 */
typedef NS_ENUM(NSUInteger, HDMUserTrackingMode)
{
 ...
 *  @note This delegate is mainly used for loading internal resources.
 */
 ...
 *  @note Before this call no API call to the map is accepted
 */
 ...
 *  @note Calls each time the user pans the map, pinchs, etc.
 */
 ...
 *  @note         Called when the map is tapped
 */
 ...
 *                      (topmost visible object is at array index n-1).
 *  @note         Called when the map is tapped at a location for more than 1 second
 */
 ...
 *  @note         Called after the map was rotated
 */
 ...
 *
 *  @param mapView   The map view that was rotated
 *  @note         Called after the map was panned
 */
 ...
 *  @note         Called after the map was zoomed
 */
 ...
 *  @note         Called after userTrackingMode was chagned
 */
 ...
 *  @note A map view sends messages to its delegate regarding the loading of map data, tapping features on the map or change point of view.
 *
 ...
 *
 ...
 @note Setting a different style forces the map to reinitialize.
 */
 ...
 *
 ...
 @note Setting different rules forces the map to reinitialize.
 */
@property (nonatomic, copy, nonnull) NSString *ruleFile;

/**
 Show/hide compass view.
 */
@property (nonatomic, assign) BOOL showsCompass;


/**
 Rerepence to compass view
 */
 ...
 */
- (BOOL)isInsideMap:(HDMLocation* _Nonnull)location NS_SWIFT_NAME(isInsideMap(location:));
...
/**
 ...
 @note Changing the value of this property updates the map view with an animated
...
/**
 Move map's camera to specified region.

 @param region Required visible region of the map.
 @param animated If YES, animate Map moving to required region If NO, show required map region without any animations.
...
/**
 ...
 @note Doesn't affect map if coordinates out of map bounds or if projection source is unknown
 */
 ...
 *
 *  @note Doesn't affect map if featureId wasn't found. Feature's featureId comes in map DB
 */
 ...
 *  @param animated If YES, animate Map moving to required obejct If NO, show required map region without any animations.
 *  @note Doesn't affect map if featureIds weren't found.
 */
- (void)moveToFeaturesWithIds:(NSArray<NSNumber*>* _Nonnull)ids animated:(BOOL)animated NS_REFINED_FOR_SWIFT;
...
/**
 * @returns the location of a point on the map given a point on the screen
 */
-(HDMLocation*)getLocationOnMapFrom:(float)screenPointX screenPointY:(float)screenPointY;
...
/**
 Add a 3d polygon to the map.
 */
-(uint64_t)createPolygonFeature:(NSArray* _Nonnull)points withType:(NSString* _Nonnull)ftypename zmin:(float)zmin zmax:(float)zmax;
...
/**
 ...
 @note Default value is YES (zoom is enabled)
 */
@property (nonatomic, assign) BOOL zoomEnabled;

/**
 *  Enable or disable scrolling(map moving)
 *   @note Default value is YES (scrolling is enabled)
 */
@property (nonatomic, assign) BOOL scrollEnabled;

/**
 *  Enable or disable rotation
 *  @note Default value is YES (rotation is enabled)
 */
