...
/**
 ...
 @note  Regardless of the elevation mode, values are always in meters.
 */
typedef NS_ENUM(NSInteger, HDMElevationMode)
{
    ///Local
    HDMElevationModeLocal,
    ///Elevation is the ellipsoidal height based on WGS84.
    HDMElevationModeWGS84
};


/**
 Class provide API for coordinate projection
 */
@interface HDMProjector : NSObject


/**
 Initialize HDMProjector instance

 @param apiCRS Coordinate reference system (CRS) to be used for all data and values passed to and returned by the map.
 @param displayCRS Set’s the projection to be used to *display* the map.
 @param elevationMode The elevation mode to be used by the API.
 @return Initialized HDMProjector instance
 */
 ...
 *
 @note This does *not* change the CRS used to render the map. See setDisplayCRS to change the
